...
upload_backup() {
    ...
    # Upload ke server backup (contoh menggunakan gist atau file.io)
    # Untuk contoh, kita akan menyimpan di local dan membuat link download via simple-http-server
    # Atau bisa menggunakan file.io API untuk upload
...
install_zivpn() {
    ...
    apt-get install -y wget curl openssl iptables ufw cron tar > /dev/null 2>&1
...
update_script() {
    ...
    local SCRIPT_URL="https://raw.githubusercontent.com/script-VIP/Vip/main/udp/ziiv.sh"
...
main_menu() {
    while true; do
        ...
        if ! is_installed; then
            ...
            echo ""
            echo -e "${WHITE}  ────────────────────────────────────────${NC}"
            read -rp "$(echo -e "  ${WHITE}Pilih menu [1-12] : ${NC}")" choice
