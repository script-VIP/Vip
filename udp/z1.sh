...
# === VARIABEL CACHE (untuk menghindari kedap-kedip) ===
CACHED_IP=""
CACHED_DOMAIN=""
CACHED_ISP=""
CACHED_COUNTRY=""
...
update_cache() {
    ...
    if [[ $((current_time - CACHE_LAST_UPDATE)) -gt $CACHE_DURATION ]] || [[ -z "$CACHED_IP" ]]; then
        CACHED_IP=$(curl -4 -s ifconfig.me 2>/dev/null || curl -4 -s icanhazip.com 2>/dev/null || hostname -I | awk '{print $1}')
        
        # Domain
        CACHED_DOMAIN=$(dig +short -x "$CACHED_IP" 2>/dev/null | head -n1 | sed 's/\.$//')
        if [[ -z "$CACHED_DOMAIN" ]]; then
            CACHED_DOMAIN=$(nslookup "$CACHED_IP" 2>/dev/null | grep 'name =' | awk '{print $4}' | sed 's/\.$//')
        fi
        if [[ -z "$CACHED_DOMAIN" ]]; then
            CACHED_DOMAIN="$CACHED_IP"
        fi
        
        # ISP dan Lokasi
        local ip_info=$(curl -s "http://ip-api.com/json/$CACHED_IP?fields=isp,country,city" 2>/dev/null)
        if [[ -n "$ip_info" ]]; then
            CACHED_ISP=$(echo "$ip_info" | grep -o '"isp":"[^"]*"' | cut -d'"' -f4)
            CACHED_COUNTRY=$(echo "$ip_info" | grep -o '"country":"[^"]*"' | cut -d'"' -f4)
            CACHED_CITY=$(echo "$ip_info" | grep -o '"city":"[^"]*"' | cut -d'"' -f4)
        fi
        
        CACHED_ISP=${CACHED_ISP:-"Unknown ISP"}
        CACHED_COUNTRY=${CACHED_COUNTRY:-"Unknown"}
        CACHED_CITY=${CACHED_CITY:-"Unknown"}
...
banner() {
    ...
    local ip=$CACHED_IP
    local domain=$CACHED_DOMAIN
    local isp=$CACHED_ISP
    local country=$CACHED_COUNTRY
    local city=$CACHED_CITY
...
main_menu() {
    while true; do
        ...
        if ! is_installed; then
            ...
            echo ""
            echo -e "${WHITE}  ────────────────────────────────────────${NC}"
            read -rp "$(echo -e "  ${WHITE}Pilih menu [1-11] : ${NC}")" choice
