#!/bin/bash
# =============================================
#   ZIVPN UDP MANAGER - FIXED VERSION
# =============================================
...
show_menu() {
    ...
    echo -e "  ${YELLOW}[09]${NC} 🤖  Atur Notifikasi Bot"
    echo -e "  ${YELLOW}[10]${NC} 🌐  Change Domain"
...
send_file() {
    ...
    if [ -f "$BOT_CONFIG" ]; then
        ...
        echo -e "${YELLOW}Silakan atur bot dulu di menu [09]${NC}"
    fi
}
    ...
    case $subchoice in
        1)
            ...
            if [ $? -eq 0 ]; then
                ...
                send_file "$BACKUP_FILE" "Backup ZIVPN - $DOMAIN_NAME - $(date +'%Y-%m-%d %H:%M:%S')"
            else
        ...
        2)
            ...
            if [ -d "$ZIVPN_DIR/backup" ] && [ "$(ls -A $ZIVPN_DIR/backup 2>/dev/null)" ]; then
                ls -lh "$ZIVPN_DIR/backup/" | grep tar.gz | awk '{print "  " $9 " (" $5 ")"}'
            else
        ...
        3)
            ...
            echo ""
            read -p "Masukkan nama file backup: " filename
    ...
    if [ -f "$BOT_CONFIG" ]; then
        ...
        if [ -n "$BOT_TOKEN" ] && [ -n "$CHAT_ID" ]; then
            curl -s -X POST "https://api.telegram.org/bot${BOT_TOKEN}/sendDocument" \
                 ...
                 -F "caption=✅ Auto Backup ZIVPN - ${DOMAIN_NAME} - $(date +'%Y-%m-%d %H:%M:%S')" > /dev/null
        fi
    fi
fi
EOF
